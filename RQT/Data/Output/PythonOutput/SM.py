...
class RGEsolver():
    ...
    def __init__(self, name, initialScale = 0, tmin = 0, tmax = 20):
        ...
        # Yukawa Couplings
        self.Yu = Coupling('Yu', 'Yukawas', latex='Y_u', shape=(3,3))
        self.Yd = Coupling('Yd', 'Yukawas', latex='Y_d', shape=(3,3))
        self.Ye = Coupling('Ye', 'Yukawas', latex='Y_e', shape=(3,3))
    ...
    def betaFunction(self, t, couplingsArray):
        ...
        if self.loops['GaugeCouplings'] >= 1:
            bg1 += beta_g1(1, g1,g2,g3,Yu,Yd,Ye,lambda_)*self.kappa(1)*np.log(10)
            bg2 += beta_g2(1, g2,g1,g3,Yu,Yd,Ye,lambda_)*self.kappa(1)*np.log(10)
            bg3 += beta_g3(1, g3,g1,g2,Yu,Yd,Ye)*self.kappa(1)*np.log(10)
        if self.loops['GaugeCouplings'] >= 2:
            bg1 += beta_g1(2, g1,g2,g3,Yu,Yd,Ye,lambda_)*self.kappa(2)*np.log(10)
            bg2 += beta_g2(2, g2,g1,g3,Yu,Yd,Ye,lambda_)*self.kappa(2)*np.log(10)
            bg3 += beta_g3(2, g3,g1,g2,Yu,Yd,Ye)*self.kappa(2)*np.log(10)
        if self.loops['GaugeCouplings'] >= 3:
            bg1 += beta_g1(3, g1,g2,g3,Yu,Yd,Ye,lambda_)*self.kappa(3)*np.log(10)
            bg2 += beta_g2(3, g2,g1,g3,Yu,Yd,Ye,lambda_)*self.kappa(3)*np.log(10)
            bg3 += beta_g3(3, g3,g1,g2,Yu,Yd,Ye)*self.kappa(3)*np.log(10)
        ...
        if self.loops['ScalarMasses'] >= 1:
            bmu += beta_mu(1, g1,g2,Yu,Yd,Ye,lambda_,mu,g3)*self.kappa(1)*np.log(10)
        if self.loops['ScalarMasses'] >= 2:
            bmu += beta_mu(2, g1,g2,Yu,Yd,Ye,lambda_,mu,g3)*self.kappa(2)*np.log(10)

        if self.loops['Vevs'] >= 1:
            bv += beta_v(1, g1,g2,Yu,Yd,Ye,v,self.xiGauge,g3,lambda_)*self.kappa(1)*np.log(10)
        if self.loops['Vevs'] >= 2:
            bv += beta_v(2, g1,g2,Yu,Yd,Ye,v,self.xiGauge,g3,lambda_)*self.kappa(2)*np.log(10)

        return [bg1, bg2, bg3, *bYu.flat, *bYd.flat, *bYe.flat, blambda_, bmu, bv]
